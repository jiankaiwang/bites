...
function change_origin_url() {
    ...
    var base = "https://bites.cdc.gov.tw/objectdetection/odres/odiptimg"
...
function change_result_complete() {
    var base = "https://bites.cdc.gov.tw/objectdetection/odres/odresimg"
      , sessUrl = "?task=" + odTaskInfo['sessname'] + "&key=" + odTaskInfo['key'];
    $('#resimgbody').attr('src',base + sessUrl);    
}

function wait_for_od_complete(retryCnt) {
    var base = "https://bites.cdc.gov.tw/objectdetection/odres"
      , sessUrl = "?task=" + odTaskInfo['sessname'] + "&key=" + odTaskInfo['key'];
...
$(function(){
  $('#odupload').on('click', function() {
    $.ajax({
      url: 'https://bites.cdc.gov.tw/objectdetection',
      type: 'POST',
